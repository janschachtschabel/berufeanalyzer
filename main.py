...
with st.sidebar:
    ...
    with st.expander("Konverter Konfiguration", expanded=False):
        ...
        selected_converter = st.selectbox(
            "Wähle das Dokumenten-Konvertierungstool",
            converter_options,
            index=0  # IBM Docling als Standard
        )
        ...
        if "IBMDocling" in selected_converter:
            ...
            allowed_extensions = [
                ...
                '.jpg', '.jpeg', '.png', '.bmp', '.gif', '.tiff', '.pnm', '.pgm', '.pbm',
                '.ppm', '.pam', '.jxr', '.jpx', '.jp2', '.psd', '.md'
            ]
        else:  # PDFPlumber
            allowed_extensions = ['.pdf', '.md']

    # Daten Konfiguration
    with st.expander("Daten Konfiguration", expanded=False):
        ...
        if os.path.exists(data_folder):
            ...
            if files_in_data:
                ...
                if st.button("Ausgewählte Dateien löschen"):
                ...
                st.info("Keine Dateien im Datenordner vorhanden")
